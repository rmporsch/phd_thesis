...
Generally these models assume that twins do not differ from the general population and that mating occurs at random.
Furthermore, it is assumed that MZ and DZ twin pairs share their corresponding environment to the same degree, also called equal environment assumption (EEA), and that the effect of gene-environment interactions are small~\cite{Rijsdijk2002}.

A number of studies have investigated and tested these underlying assumptions.
Especially the EEA has been questioned~\cite{Martin1997}. 
...
Thus suggesting that the assumptions underlying twin modeling hold.

However, despite the numerous benefits of twin modeling, new advances in molecular technology has shifted research towards the identification of specific genetic loci.
In the following section I will outline the basics of the beginnings of molecular genetics with the introduction of linkage studies.
I will then describe the basics of \acrfull{gwas} and the more recently developed \acrfull{ngs}.

\section{Linkage Studies}
\label{sec:linkage_studies}

Previous efforts to identify specific genetic markers associated with certain diseases made use of linkage studies.
Linkage studies make use of the fact that many genes, which are in close proximity to each other, are inherited together during meiosis.
The method is use to identify genetic markers which co-segregate with a particular gene of interest.
This allows to easily track genes throughout a family.
Thus the presents of the maker in affected, but absents healthy family members allows the identification of disease associated genes~\cite{DawnTeare2005}. 
This classical genetic method has been been successful in identifying many monogenic diseases~\cite{Londin2013}.

However, due to the low resolution linkage studies have not been very successful in identifying risk loci for more complex phenotypes.
Thus modern genetic studies make use genotyping arrays to identify specific SNPs associated with diseases and other phenotypes.
...
However, the concept of linkage does also extent to SNPs, also called \acrfull{ld}.

\section{Linkage Disequilibrium}
\label{sec:linkage_disequilibrium}

Indeed, before outlying the concept of modern association studies in more detail it is important to mention \acrfull{ld}.
...
\label{sec:common_versus_rare_genetic_variants}
