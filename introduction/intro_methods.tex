...
Association analysis of common genetic variants are usually only applied to investiagte potential genetic markers of common traits and disorders.
...
This was made possible with a techique called \textit{linkage analysis}, which made use of the segration of genetic markers across multiple families affected with the disorder~\cite{Kerem1989}.
However, while this method was initally successful with a number of other rare disorders, such as Huntignton, common disorder, such as heard disease, psychiatric disorders as well as other, fared not that well.  
...
Specically, it suggests that common disorders are affected by genetic variantions which are also common in the population.
...
Specically, a common variants with a frequency of 45\% that directly cause a disorder would mean that also 45\% of the population would be affected by the disease.
This is unlikely to be the case and it is more likely that a given SNP only has a small effect of the disease.
For example, an SNP might only slightly alter the gene expression of a particular gene and increase the overal risk for a disease.
Implying that the correlation between the allele frequency and disease prevalence will be low.
Thus, suggesting that common variants are unlilkely to have a high penetrance.

This in turn also implies that multiple common variants affect a comon disorder.
For example, twin studies have estimated the heritability of aggressive behavior at 50\%.
If a single SNP has only a samll effect on the phenotype, then the total genetic risk must be distributed across multiple common genetic variants.
...
However this does suggest that all gentic variants of a common disorder is due to common SNPs allone.
...
Nevertheless, during the last decade the cost of obtaining a person's genome reduced rapidly.
...
This allows the flexible inclusion of multiple covariates in order to adjust for possible confounding factors such as population stratification, sex, genotyping chip and others.
