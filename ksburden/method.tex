...
Thus one can define the \acrlong{ECDF} $F(x)$ for $G$ as
\begin{equation}
  F(x) = \frac{1}{\Gamma} \sum^P_{p=1}(I(p \leq x)\sum^n_{i=1}G_{ip})
\end{equation}
in which $\Gamma = \sum^P_{p=1}\sum^n_{i=1} G_{ip}$ and $I(x)$ is the indicator function for a fixed position $x$.
