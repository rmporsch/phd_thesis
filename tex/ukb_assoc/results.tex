...
No genome wide significant loci were identified in $37,320$ subjects (see Figure~\ref{fig:gwas_impAgg}).
...
\begin{table}[!htpb]
	\centering
	\input{ukb_assoc/tables/ldsc/ldscore_results_4526.tex}
...
The genome wide association study on risk taking yield two genome wide significant loci in $120,286$ subjects.
The Manhattan plot in figure~\ref{fig:riskmanhatten} shows two signal in chromosome 3 and 6, as well as a signal which is just below genome wide significant at chromosome 4 (see also Table~\ref{tab:lead_snps_risk}).
...
\begin{table}[!htpb]
	\centering
	\input{ukb_assoc/tables/ldsc/ldscore_results_2040.tex}
...
\begin{table}
	\small
	\centering
	\input{ukb_assoc/tables/lead_snps/lead_snp_2040.tex}
	\caption{Lead SNPs reaching genome wide significance in Risk Taking}\label{tab:lead_snps_risk}
\end{table}

Closer inspection of the signal in chromosome 3 shows 2 lead SNPs namly \textit{rs1308431} and \textit{rs7639518}.
However, \textit{rs1308431} and \textit{rs7639518} have opposite directions of effect.
In order to investigate the signals are independency a conditional analysis was performed.
...
However, a closer look at the conditional regression model suggest that the \textit{rs7639518} is approaching genome wide significant levels (see table~\ref{tab:conditional}). 

\begin{table}[!htpb]
	\small
	\centering
	\resizebox{\textwidth}{!}{\input{ukb_assoc/tables/conditional_analysis_rs13084531.tex}}
...
\begin{table}[!htpb]
	\centering
	\resizebox{\textwidth}{!}{\input{ukb_assoc/tables/gwas_catalog_2040.tex}}
