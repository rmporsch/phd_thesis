...
The association between an SNP and a trait can be expressed as a linear regression model for a continuous and binary phenotype respectively
...
Rare variants, commonly with a \acrfull{maf} of equal or below 1\%, have been suggested to explain the bulk of the missing heritability~\cite{Jiang2013,Li2009a}.
The drop in sequencing costs has allowed to investigate these rare variants~\cite{Goodwin2016}.
There are, nevertheless, a number of problems associated with analysis of rare mutations.
While cost has dropped whole-exome and whole-genome sequencing, acquiring large set of samples remains expensive.
...
In GWAS single variants are associated with the trait in question.
...
In general one can divide these combination approaches into burden and variance component tests.
...
The simplest approach, called the burden test, tests the weighted sum of the combined scores
