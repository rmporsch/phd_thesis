...
Summary statistics of 4 large genome wide association studies were used to compute genetic correlations as well as estimated causal effects via Mendelian randomization.
...
Summary statistics for Schizophrenia was taken from a study by~\citet{Ripke2014} which assessed 36,989 cases and 113,075 controls.
...
In addition participants with `white European' ancestry identified by a k-mean closer of the first four principle components of genotype data were included.
...
The exposure is a function of the genetic variant, confounder and an independent error $\epsilon_i^X$ 
and the effect of each variant $j$ on the exposure is represented by $\gamma_j$.
The outcome is the result of the linear function of the genetic variants, the exposure, the confounder as well as an error term ($\epsilon_i^Y$).
The to estimate causal effect between exposure and outcome is $\beta$, while $\alpha_j$ represents the undesired direct effect between the genetic variant $j$ to the outcome.
...
However, single variants have often inadequate statistical power, so on can extend Equation~\ref{eq:causal_estiamte} to multiple variants as a weighted average of multiple ratio estimates across uncorrelated genetic variants~\cite{Bowden2015}.
\begin{equation} \label{eq:IVW}
  \frac{\sum^J_{j=1} \hat{\gamma}_j^2\sigma_{Yj}^{-2} \hat{\beta}_j}
  {\sum^J_{j=1} \hat{\gamma}_j^2\sigma_{Yj}^{-2}}
\end{equation}
In which $\hat{\beta}_j = \frac{\hat{\Gamma}_j}{\hat{\gamma}_j}$ and often the chosen weight $\sigma_{Yj}$ is the standard error of the outcome on the $jth$ variant.

However, often one cannot completely exclude that $\alpha_j \neq 0$.
...
Hence Equation~\ref{eq:IVW} is then re-written as
...
\end{equation}
Importantly this implies that the assumption of independence between genetic variants with outcome $y$ conditional on $X$ holds if the bias term is equal to zero.
...
Following, estimates are then ranked and the median of this distribution is taken to the estimated causal effect between exposure and outcome.
The simple approach has the benefit that if at least 50\% of variants are valid instrument this method will give consistent causal estimates.
...
MR-egger makes use of this by recessing $\hat{\Gamma}_j$ on $\hat{\gamma}_j$
\begin{equation}\label{eq:egger}
  \hat{\Gamma}_j = \beta_{0E} + \beta_{E} \hat{\gamma}_j
\end{equation}
Interestingly, testing if the intercept $\beta_E$ differs from $0$, or in other words testing if weaker instruments are upward or downward biased, also gives an indication of the overall average pleiotropy.
